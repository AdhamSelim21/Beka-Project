  ...
  return (
    <div>
          ...
          {gymBase.map((gyms) => (
            ...
            >
              <div className="aspect-[3/4] bg-surface-variant overflow-hidden rounded-2xl mb-0">
                <Image
                  className="w-full h-full object-cover transition-all duration-700 rounded-lg group-hover:scale-105 transition-transform duration-500 cursor-pointer"
                  src={gyms.image}
                  alt={gyms.title}
                />
              </div>
      ...
      <section className="max-w-7xl mx-auto px-8 py-24">
        ...
        {gyms.docs && gyms.docs.length > 0 ? (
          gyms.docs.map((gym) => (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6" key={gym.id}>
              <div className="aspect-[16/9] bg-surface-container-highest rounded-[2.5rem] overflow-hidden cursor-pointer">
                <Image
                  ...
                  src={
                    (gym.image as Media).url || 'https://via.placeholder.com/400x600?text=No+Image'
                  }
